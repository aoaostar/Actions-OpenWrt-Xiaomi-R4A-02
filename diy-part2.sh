...
# 版本号里显示一个自己的名字（Pluto build $(TZ=UTC-8 date "+%Y.%m.%d") @ 这些都是后增加的）
sed -i "s/OpenWrt /Pluto build $(TZ=UTC-8 date "+%Y.%m.%d") @ aoaostar /g" package/lean/default-settings/files/zzz-default-settings
